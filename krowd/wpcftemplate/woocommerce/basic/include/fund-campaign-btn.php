...
<div class="wpneo-single-sidebar ASA">
	...
	// var_dump( $post );
	// die();
	$extra_link = get_post_meta( $post->ID, 'krowd_project_link', true ); 
	$currency = '$';
	...
	if ($product->get_type() == 'crowdfunding') {
		if ( wpcf_function()->is_campaign_valid( $post->ID ) ) {
            ...
            <span class="wpneo-tooltip">
                <span class="wpneo-tooltip-min"><?php _e('Minimum amount is ','krowd'); echo $currency.$min_price; ?></span>
                <span class="wpneo-tooltip-max"><?php _e('Maximum amount is ','krowd'); echo $currency.$max_price; ?></span>
            </span>
            ...
            <form enctype="multipart/form-data" method="post" class="cart AA">
                ...
                <button type="submit" class="<?php echo apply_filters('add_to_donate_button_class', 'btn-back-campaign btn-theme-2 wpneo_donate_button'); ?>"><span><?php _e('Back Campaign', 'krowd'); ?></span></button>
            </form>
         <?php }else{ ?>
		 ...
		 <?php } else {
			if ( ! wpcf_function()->is_campaign_started()){
				...
				if( wpcf_function()->is_reach_target_goal( $post->ID ) ){
					_e('The campaign is successful.','krowd');
				}else{
					_e('This campaign has been invalid or not started yet.','krowd');
				}
			}
		}
	}
